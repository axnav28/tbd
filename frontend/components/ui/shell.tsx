...
export function Shell({ eyebrow, title, children }: { eyebrow: string; title: string; children: React.ReactNode }) {
  return <div className="app-shell"><aside><div className="brand"><span className="brand-mark">T</span><span>TBD<span className="brand-muted"> / RISK OS</span></span></div><div className="tenant"><span className="live-dot" /> NORTHSTAR FINANCE <small>SYNTHETIC TENANT</small></div><nav>{links.map(([href, label, Icon]) => <Link key={href} href={href}><Icon size={16} />{label}</Link>)}</nav><div className="rail-note"><strong><span className="live-dot" />LIVE MONITORING</strong>Continuous attack-path evaluation<br /><span>Engine v0.6 · India region</span></div></aside><main className="content"><header><div><div className="eyebrow">{eyebrow}</div><h1>{title}</h1></div><div className="header-right"><div className="live-pill"><span className="live-dot" /> LIVE MODEL <small>60s refresh</small></div></div></header>{children}</main></div>;
}
